...
with tab1:
    ...
    st.text('The purpose of this application is to predict if a user/patient is at high risk for developing diabetes using an adaptable machine learning model. ')
...
with tab2:
    ...
    with st.form(key='data_form_1'):
        ...
        st.header('STEP 1: Please select datafile:')
        ...
        #Put in a statement to press Load File Button
        st.subheader('AFTER SELECTING THE DATAFILE PLEASE PRESS "LOAD FILE" BUTTON')
...
with tab3:
    ...
    st.header('STEP 2: Please select your y column (what you would like to predict based on the data)')
    ...
    with st.form(key='feature_label_select_1'):
        ...
        st.header('Selection Your y column (In current dataset we adivse to use OUTCOME column)')
        st.write('Column Outcome is the column that displays if a person has Diabetic (1) or Non-Diabetic (0)')
...
with tab4:
    ...
    st.subheader('STEP 3: Prepare Data for Machine Learning Model')
    st.subheader(f'Current Selected y Column is: {label_select}')
...
# Create a Form to make changes to the original data
    with st.form(key='Data_select_1'):
        st.header('STEP 3.1: Data Enhancements')

        # Remove any of the columns that "seem" insignificant don't have much correlation with Y column
        st.subheader('1. Please remove any X features (columns) that should not be taken into account, and press Select Features Button')
    ...
    st.header(f'STEP 3.2: Categorical Variables Handling. (If none in dataset, please proceed to Standard Scaler Option)')
    ...
    with st.form(key='Data_select_3'):
        ...
        st.header('STEP 3.3.1: Please Select if you like to apply Standard Scaler to the Dataset?')
        ...
        st.header('STEP 3.3.2: Please Select the Percentage Amount of data to be used as test data and press APPLY button')
    ...
    with st.form(key='Data_select_5'):
        st.header('STEP 3.4: Please select if you would like to apply OverSampling/Undersampling or None (No Sampling) and press APPLY')
    ...
    st.text('PLEASE PROCEED TO STEP 4 USER INPUT tab')
...
with tab5:
    ...
    st.title('STEP 4: Patient prediction input form')
    ...
    with st.form('flex_form'):
        ...
        # create input slider for gluvose bloodpressure and insuline
            if model_feature == 'Glucose' or model_feature == 'BloodPressure':
                ...
                submitted = str('submitted') + str(model_feature_number)
                submitted = st.slider(label=model_feature,min_value=0,max_value=700,value=350,step=1,key=model_feature_number)
                #st.write(submitted)
                answers.append(submitted)
            # create input slider from BMI skinthinkness and age
                ...
                submitted = str('submitted') + str(model_feature_number)
                submitted = st.slider(label=model_feature,min_value=float(0),max_value=float(5),value=float(2.5),step=float(0.001),key=model_feature_number)
                #st.write(submitted)
                answers.append(submitted)
        ...
        # Create a line where it says to press the save/apply button
        st.write('Please input Patient data and press Save/Apply button')
...
with tab6:
    ...
    with st.form(key='neural_net_feat_1'):
        ...
        st.header('STEP 6.1: Please select purpose of Model and press Model Purpose Select button')
    ...
    with st.form(key='nn_feat_2'):
        st.header('STEP 6.2: Please select below Neural Network Options')
        if compile_loss_select == 'Probalistic':
            st.subheader('Neural Network Probalistic Options')
            ...
            st.subheader('Neural Network Regression Options')
        ...
        st.subheader('!! PLEASE SELECT AMOUNT OF EPOCHS YOU WOULD LIKE TO USE AND PRESS APPLY AND RUN AFTERWARDS (Standard value = 1) !!')
        ...
        if compile_loss_select == 'Probalistic':
            ...
            st.subheader('Current saved Neural Network Loss and Model Accuracy')
        ...
        # Evaluate the model loss and accuracy metrics using the evaluate method and the test data for regression
            ...
            st.subheader('Current saved Neural Network Loss and Model MSE')
...
with tab7:
 ...
 #LIME (Local Interpretable Modalagnostic Explanations) ML model interpretation
    ...
    lime_model=RandomForestClassifier(random_state=5)
    
    #fit model
    lime_model.fit(X_train, y_train)
    
    #80% accurate classifier created with
    st.write(classifier_score=lime_model.score(X_test, y_test))
    
    #establish parameters LIME
    lime_parameters=lime_tabular.LimeTabularExplainer( training_data = np.array(X_train), feature_names = df.columns, class_names = ["Positive", "Negative"], mode='classification')
    
    
    
    #explain prediction showing probabilities
    pred_explained = lime_parameters.explain_instance(data_row=X_test.iloc[1], predict_fn=lime_model.predict_proba)
    
    pred_explained.show_in_notebook(show_table=True)
    
    pred_explained2 = lime_parameters.explain_instance(data_row=X_test.iloc[2], predict_fn=lime_model.predict_proba)

    pred_explained2.show_in_notebook(show_table=True)
...
with tab8:
    ...
    st.write('Diabetes prediction based on user input data:')
