...
with tab6:
    ...
    with st.form(key='select_load_model_1'):
        ...
        # Select h5 files only (i.e., models)
        ...
        # Load the selected Model into tf.keras.models.load_model
        model_loaded = tf.keras.models.load_model(model_path)

        st.write('Model Loaded:', model_path)

        # Generate the model's architecture image using plot_model and save it as a temporary file
        with tempfile.NamedTemporaryFile(suffix=".png") as temp_file:
            plot_model(model_loaded, to_file=temp_file.name, show_shapes=True, show_layer_names=True)
            temp_file.flush()

            # Display the image in Streamlit
            st.image(temp_file.name, caption="Model Architecture")
